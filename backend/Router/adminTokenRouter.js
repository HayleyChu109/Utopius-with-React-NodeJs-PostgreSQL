...
class AdminTokenRouter {
  ...
  async postTokenPlan(req, res) {
    let { planName, noOfToken, detail, hkd, photoPath } = req.body;
    let tokenPlan = await this.adminTokenService.postTokenPlan(
      planName,
      noOfToken,
      hkd,
      detail,
  ...
  async postRedeemItem(req, res) {
    let { name, token, stock, itemPath } = req.body;
    let result = await this.adminTokenService.postRedeemItem(
      name,
      token,
      itemPath,
      stock
  ...
  async putRedeemItem(req, res) {
    ...
    let result = await this.adminTokenService.putRedeemItem(
      id,
      name,
      tokenCost,
      itemPic,
