...
export const Load_TOKEN_USER_TRANSACTION_SUCCESS =
  "LOAD_TOKEN_USER_TRANSACTION_SUCCESS";
...
export const GetTokenUserTransaction = () => async (dispatch) => {
  ...
  try {
    ...
    dispatch({
      type: Load_TOKEN_USER_TRANSACTION_SUCCESS,
...
export const PutRedeemItem = (itemObj, id) => async (dispatch) => {
  ...
  try {
    let response = await axios.post(
      `${process.env.REACT_APP_API_SERVER}/token/redeemItem/${id}`,
      itemObj,
