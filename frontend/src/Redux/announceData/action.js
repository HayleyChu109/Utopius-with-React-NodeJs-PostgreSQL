...
export const GetAnnouncement = (requestId) => async (dispatch) => {
    ...
    try {
      ...
      const {id,content,isPrivate,start_date,end_date,title}=response.data[0]
      console.log(content)
...
export const PostAnnouncement = (title,content,isPrivate) => async (dispatch) => {
    let userToken = localStorage.getItem("token");
    console.log(userToken);
    try {
      let response = await axios.post(
        `${process.env.REACT_APP_API_SERVER}/announce`,{title:title,content:content,isPrivate:isPrivate},
...
export const PutAnnouncement = (id,title,content,isPrivate) => async (dispatch) => {
    let userToken = localStorage.getItem("token");
    console.log(userToken);
    try {
      let response = await axios.put(
        `${process.env.REACT_APP_API_SERVER}/announce/${id}`,{title:title,content:content,isPrivate:isPrivate},
